...
- **构建**：Vite
- **测试**：后端 pytest（43 个测试）、前端 Vitest（27 个单元测试）+ Playwright（E2E）
...
│       └── src/
│           ├── views/           # 页面视图（PipelineEditor / LlmChat / ConversationHistory / SkillManager）
...
- **Build**: Vite
- **Testing**: Backend pytest (43 tests), Frontend Vitest (27 unit tests) + Playwright (E2E)
...
# Run Web backend tests (43 tests)
pip install -r web/backend/requirements.txt
pytest web/backend/tests/ -v
...
│       └── src/
│           ├── views/           # Page views (PipelineEditor / LlmChat / ConversationHistory / SkillManager)
