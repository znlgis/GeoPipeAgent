...
- **24 个内置步骤** — 覆盖 IO、矢量、栅格、空间分析、网络分析五大类
- **多后端支持** — GeoPandas (Python)、GDAL CLI (ogr2ogr)、QGIS Process 三种后端可切换
...
GeoPipeAgent 可自动生成 Skill 文件，让 AI（如 ChatGPT、Claude）理解框架能力并生成流水线。
...
然后在 `src/geopipe_agent/steps/__init__.py` 的 `load_builtin_steps()` 中添加导入即可自动注册。
...
| 后端 | 实现 | 要求 |
|------|------|------|
| `gdal-python` | GeoPandas + Shapely（默认） | pip 安装即可 |
| `gdal-cli` | ogr2ogr 命令行 | 需安装 GDAL CLI 工具 |
| `qgis-process` | QGIS Processing | 需安装 QGIS |
...
- **24 built-in steps** — Covers IO, vector, raster, spatial analysis, and network analysis
- **Multi-backend** — Switch between GeoPandas (Python), GDAL CLI (ogr2ogr), and QGIS Process
...
steps:                      # Required, list of steps
  - id: step-id             # Required, unique identifier [a-z0-9_-]
    ...
    backend: gdal-python    # Optional, specify backend
...
The `cookbook/` directory provides 5 ready-to-use pipeline examples:
...
GeoPipeAgent auto-generates Skill files so that AI assistants (e.g. ChatGPT, Claude) can understand the framework and generate pipelines.
...
1. Provide Skill files to the AI
...
Then add the import in `src/geopipe_agent/steps/__init__.py`'s `load_builtin_steps()` function for auto-registration.
...
|---------|----------------|--------------|
| `gdal-python` | GeoPandas + Shapely (default) | pip install |
| `gdal-cli` | ogr2ogr CLI | GDAL CLI tools installed |
| `qgis-process` | QGIS Processing | QGIS installed |
