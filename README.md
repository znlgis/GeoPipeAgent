...
- **多后端支持** — 五种后端可切换：Native Python (GeoPandas)、GDAL CLI、GDAL Python、QGIS Process、PyQGIS
...
- **后端**：FastAPI + Uvicorn + OpenAI SDK + SSE-Starlette
- **前端**：Vue 3 + TypeScript + Element Plus + Pinia + Vue Flow + vue-i18n
- **构建**：Vite
- **测试**：后端 pytest（28 个测试）、前端 Vitest（27 个单元测试）+ Playwright（E2E）

### API 端点

| 路径 | 说明 |
|------|------|
| `/api/pipeline/*` | 流水线操作（验证、执行、导出） |
| `/api/llm/chat` | LLM 对话（SSE 流式） |
| `/api/llm/generate` | 自然语言生成 YAML 流水线 |
| `/api/llm/analyze` | 分析流水线执行结果 |
...
│       └── src/
│           ├── views/           # 页面视图
│           ├── components/      # 组件（chat/flow/common）
│           ├── stores/          # Pinia 状态管理
│           ├── composables/     # 组合式函数
...
- **Multi-backend** — Five backends: Native Python (GeoPandas), GDAL CLI (ogr2ogr), GDAL Python, QGIS Process, and PyQGIS
...
- **Backend**: FastAPI + Uvicorn + OpenAI SDK + SSE-Starlette
- **Frontend**: Vue 3 + TypeScript + Element Plus + Pinia + Vue Flow + vue-i18n
- **Build**: Vite
- **Testing**: Backend pytest (28 tests), Frontend Vitest (27 unit tests) + Playwright (E2E)

### API Endpoints

| Path | Description |
|------|-------------|
| `/api/pipeline/*` | Pipeline operations (validate, execute, export) |
| `/api/llm/chat` | LLM conversation (SSE streaming) |
| `/api/llm/generate` | Natural language → YAML pipeline generation |
| `/api/llm/analyze` | Analyze pipeline execution results |
| `/api/llm/config` | LLM configuration (API key, model, temperature, etc.) |
| `/api/export/*` | Export pipelines |
...
# Run all core library tests (139 tests)
python -m pytest tests/ -v

# Run tests with coverage report
python -m pytest tests/ -v --cov=geopipe_agent

# Run Web backend tests (28 tests)
pip install -r web/backend/requirements.txt
pytest web/backend/tests/ -v
...
│       └── src/
│           ├── views/           # Page views
│           ├── components/      # Components (chat/flow/common)
