...
pyqgis_available = PyQgisBackend().is_available()
pytestmark = pytest.mark.skipif(not pyqgis_available, reason="PyQGIS not installed")


class TestPyQgisBackend:
    @pytest.fixture
    def backend(self):
        return PyQgisBackend()

    def test_name(self, backend):
        assert backend.name() == "pyqgis"
    ...
    def test_overlay_unsupported(self, backend, sample_polygons_gdf, sample_clip_gdf):
        with pytest.raises(ValueError, match="Unsupported overlay method"):
            backend.overlay(sample_polygons_gdf, sample_clip_gdf, how="invalid")
